describe('home page test', () => {
  ...
  describe('Approved sale', () => {
    ...
    it('Approve as employee', () => {
      ...
      cy.get('app-navbar').should('be.visible');
      cy.get(':nth-child(2) > .cdk-column-accept > .icon-button > img').click();
      cy.get('input').type('Komentaar');
      cy.get('.myButtonSecondary').contains('Potvrdi').click();
      cy.get('.mat-mdc-snack-bar-label.mdc-snackbar__label')
      .should('contain', 'Uspešno ste prihvatili.');
    });
  });
